...
// ProveSpace receives a space challenges parameter (which label of which files are included), and returns a challenge proofs, which may be error
spaceProof, err := prover.ProveSpace(spaceChals)
if err != nil {
    //error handling code...
}
...
Finally, each idle file will generate about 576 bytes of auxiliary information and 7\*FileSize of intermediate information when it is generated. They will be deleted after the commitment proof is completed, so please reserve at least 8\*FileSize+576 bytes of Real physical space to generate idle files.